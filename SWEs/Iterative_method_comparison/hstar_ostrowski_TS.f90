...
! =========================================================
    ...
    if (conv_criteria==1) then
        ...
        print *,"Iterations Ostrowski TS: ", iterations/(n_data +0.d0)
...
double precision function hStarWetStates(g,hL,hR,uL,uR,tol,iterations)
    !This function considers the case where both states are wet and gives an estimate on hStar 
    implicit none
    ...
    if (iterate==1) then 
        ...
        do while(.true.)
            ...
            if (isnan(h).or. isnan(phiR)) then
                ...
                print *, "Iterations: ", iterations_single_RP
                call abort
