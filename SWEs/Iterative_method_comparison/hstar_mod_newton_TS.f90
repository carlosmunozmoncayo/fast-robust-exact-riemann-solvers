!RS using modified Newton's method with TS initial guess
! =========================================================
...
! =========================================================
    ...
    if (conv_criteria==1) then
        ...
        !We use a convergence criteria based on the stagnation
        call cpu_time(start_time)
        do i=1,n_data
...
double precision function hStarWetStates(g,hL,hR,uL,uR,tol,iterations)
    ...
    double precision :: yz
    double precision :: phipaverage, h_half_step
    ...
    if (iterate==1) then 
        ...
        h_half_step=h
        phipaverage=phip(g,h,hL,hR)
        h=h-phi(g,h,hL,hR,uL,uR)/phip(g,h,hL,hR)
        ...
        do while(.true.)
            ...
            h_half_step=h-phiR/phipaverage
            phipaverage=phip(g,0.5d0*(h+h_half_step),hL,hR)
