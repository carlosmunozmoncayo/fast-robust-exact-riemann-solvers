! Adaptive noniterative RS for the SWEs 
! =========================================================
...
! =========================================================
    ...
    if (conv_criteria==1) then
        ...
        print *,"No iterations: ", iterations/(n_data +0.d0)
...
double precision function hStarWetStates(g,hL,hR,uL,uR,tol,iterations)
    ...
    if (0<=fMin) then 
        ...
        fMax = phi(g,hMax,hL,hR,uL,uR)
        if (fMax<0) then
            hStarWetStates = SharpInitialGuess(g,hL,hR,uL,uR)
        else
            ...
            hStarWetStates = (phiR*hMin-fMin*h)/(phiR-fMin)
        END if
    END if
...
double precision function hStarWetStates_stagnation(g,hL,hR,uL,uR,tol,iterations)
    ...
    if (0<=fMin) then 
        ...
        phiR=phi(g,h,hL,hR,uL,uR)
        h = (phiR*hMax-fMax*h)/(phiR-fMax)
        hold=h
    else
        ! Here we have one rarefaction and one shock  
        h = min(hMax,TwoRarefactionInitialGuess(g,hL,hR,uL,uR)) !upper bound, lower bound is hmin
        phiR=phi(g,h,hL,hR,uL,uR)
        h = (phiR*hMin-fMin*h)/(phiR-fMin)
        hold=h
    END if
    ...
    if (iterate==1) then 
        ...
        do while(.true.)
            ...
            if (isnan(h).or. isnan(phiR)) then
                print *, "Aborted because of Nan, hstar reached"
