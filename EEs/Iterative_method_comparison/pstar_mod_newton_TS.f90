! Exact RS Euler Equations using Newton's modified method with TS initial guess
...
! =========================================================
    ...
    if (conv_criteria==1) then
        ...
        print *,"Iterations Mod Newton TS: ", iterations/(n_data +0.d0)
        ...
        call cpu_time(finish_time)
        exec_time=finish_time-start_time
        print *,"Iterations Newton TS stagnation criteria: ", iterations/(n_data +0.d0)
...
double precision function pStarNonVacuum(gamma,pL,uL,rhoL,pR,uR,rhoR,aL,aR,tol,iterations) 
    ...
    double precision :: p_half_step, phipaverage
    ...
    if (iterate==1) then 
        ...
        p_half_step=p
        phipaverage=phip(gamma,p,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR)
        !p = max(pMin,p-phi(gamma,p,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR)/&
        !phip(gamma,p,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR))  
        p = p-phi(gamma,p,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR)/&
        phip(gamma,p,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR) 
        ...
        if (abs(phiR)<tol) then 
           exit
        END if
        p_half_step=p-phiR/phipaverage
        phipaverage=phip(gamma,0.5d0*(p+p_half_step),pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR)
        p=p-phiR/phipaverage
        ...
        if (isnan(p).or.isnan(phiR)) then 
            print *, "aborted by Nan at", iterations_single_RP, "iterations"
