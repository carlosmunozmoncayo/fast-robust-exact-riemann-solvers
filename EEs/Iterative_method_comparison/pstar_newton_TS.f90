...
double precision function pStarNonVacuum(gamma,pL,uL,rhoL,pR,uR,rhoR,aL,aR,tol,iterations) 
    ...
    double precision ::pMin, phiMin, TwoRarefactionInitialGuess, phi, phip,  ustar, extra
    ...
    if (iterate==1) then 
        ...
        pStarNonVacuum = p
    END if
    extra=uStar(gamma,pStarNonVacuum,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR)
...
double precision function pStarNonVacuum_stagnation(gamma,pL,uL,rhoL,pR,uR,rhoR,aL,aR,tol,iterations) 
    ...
    END if
    extra=uStar(gamma,pStarNonVacuum_stagnation,pL,uL,rhoL,pR,uR,rhoR,aL,aR,CL,CR,BL,BR)
...
double precision function rhoStar(gamma, p, pZ, uZ, rhoZ)
    !Here we assume that pStar has been found
    implicit none
    double precision :: gamma, p, pZ, uZ,rhoZ, numer,denom
    
    if(p>pZ) then !Wave is shock
        rhoStar=rhoZ*((p/pZ+(gamma-1)/(gamma+1))/((gamma-1)*p/(gamma+1)/pZ+1))
    else !Wave is rarefaction
        rhoStar=rhoZ*(p/pZ)**(1/gamma)
    END if
