...
def setup(use_petsc=False,outdir='./_output',solver_type='classic',riemann_solver="exact",kernel_language='Fortran',mx=4050,order=1,tfluct_solver=False):
    ...
    state.q[density ,:] = 0.1
    state.q[momentum,:] = 0.
    #state.q[energy  ,:] = ( (x<0.5)*3000 + (0.5<=x)*1.e-8) / (gamma - 1.)
