! Exact RS Euler equations using Newton's method with Linearized initial guess
! =========================================================
...
! =========================================================
    ...
    if (conv_criteria==1) then
        ...
        print *,"Iterations Newton PV: ", iterations/(n_data +0.d0)
        ...
        call cpu_time(finish_time)
        exec_time=finish_time-start_time
        print *,"Iterations Newton PV stagnation criteria: ", iterations/(n_data +0.d0)
...
double precision function pStarNonVacuum(gamma,pL,uL,rhoL,pR,uR,rhoR,aL,aR,tol,iterations) 
    ...
    if (0<=phiMin) then 
        ...
        p=max(pMin,0.5d0*(pL+pR)-0.125d0*(uR-uL)*(rhoL+rhoR)*(aL+aR))
    END if
